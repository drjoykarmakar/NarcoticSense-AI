...
def peak_table(spectrum: Spectrum, *, prominence: float = 0.05, max_peaks: int = 50) -> pd.DataFrame:
    ...
    rows = []
    for rank, idx in enumerate(sorted(range(len(peaks)), key=lambda i: spectrum.y[peaks[i]], reverse=True)[:max_peaks], start=1):
        peak_idx = peaks[idx]
        rows.append({
            "rank": rank,
